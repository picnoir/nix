...
#define DTRACE_EVAL_IN(evalState, probeName) \
    ...
    long dtrace_exprId = evalState.exprId; \
    evalState.exprId += 1; \
    DTRACE_PROBE4(nix, probeName, dtrace_exprId, dtrace_pos.line, dtrace_pos.column, dtrace_pos.file.c_str());

// note: DTRACE_EVAL_OUT should *always* be expanded in a scope where
// a DTRACE_EVAL_IN has already been expanded.
#define DTRACE_EVAL_OUT(probeName) \
    DTRACE_PROBE4(nix, probeName, dtrace_exprId, dtrace_pos.line, dtrace_pos.column, dtrace_pos.file.c_str());
...
void EvalState::eval(Expr * e, Value & v)
{
    ...
    long dtrace_exprId = this->exprId;
    this->exprId++;
    DTRACE_PROBE4(nix, top_level__in, dtrace_exprId, dtrace_pos.line, dtrace_pos.column, dtrace_pos.file.c_str());

    e->eval(*this, baseEnv, v);

    DTRACE_PROBE4(nix, top_level__out, dtrace_exprId, dtrace_pos.line, dtrace_pos.column, dtrace_pos.file.c_str());
}
        ...
        for (auto & i : attrPath) {
            ...
            if (def) {
                    ...
                    (j = vAttrs->attrs->find(name)) == vAttrs->attrs->end())
                {
                    ...
                    DTRACE_EVAL_OUT(select_short__out);
                    return;
                }
            } else {
...
void ExprOpHasAttr::eval(EvalState & state, Env & env, Value & v)
{
    ...
    for (auto & i : attrPath) {
        ...
        {
            v.mkBool(false);
            DTRACE_EVAL_OUT(has_attr_failed__out);
            return;
        } else {
            vAttrs = j->value;
        }
...
void ExprCall::eval(EvalState & state, Env & env, Value & v)
{
    ...
    try {
        ...
        DTRACE_EVAL_OUT(call__out);
    } catch (AssertionError &e) {
        DTRACE_EVAL_OUT(call_throwned__out);
        throw e;
    }
}
...
void ExprOpUpdate::eval(EvalState & state, Env & env, Value & v)
{
    ...
    if (v1.attrs->size() == 0) {
        v = v2;
        DTRACE_EVAL_OUT(op_update_empty1__out);
        return;
    }
    if (v2.attrs->size() == 0) {
        v = v1;
        DTRACE_EVAL_OUT(op_update_empty2__out);
        return;
    }
    ...
    DTRACE_EVAL_OUT(op_update__out);
}
